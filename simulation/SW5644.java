...
public class SW5644 {
	...
	public static void main(String[] args) throws IOException {
		System.setIn( new FileInputStream("simulation/SW5644.txt"));
	...
	static void solve() {
		for (int time = 0; time <= M; ++time) { // 처음 있는 자리부터 처리해야하므로 시간은 0부터
			 ...
			 * 
			 * A B 충전량 A만 있는 경우 BC1, 0 0, 0 BC1 + 0 B만 있는 경우 0, 0 BC1, 0 BC1 + 0 A와 B가 다른 영역
			 * BC1, 0 BC2, 0 BC1 + BC2
			 * 
			 * 같은 영역일 경우는 가장 큰 영역을 선택하고 다른 영역중 큰 값을 선택한다. 최대충전 같은 영역 BC1, 0 BC1, 0 BC1 +
			 * max(BC2, BC3) 최대충전 같은 영역 BC1, BC2 BC1, BC3 BC1 + max(BC2, BC3) 최대충전 같은 영역
			 * BC1, BC2 BC1, 0 BC1 + max(BC2, BC3) 최대충전 같은 영역 BC1, 0 BC1, BC3 BC1 + max(BC2,
			 * BC3)
			 */
